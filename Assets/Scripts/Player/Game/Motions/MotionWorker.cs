        ...
        public void StartDefaultMotion(float duration)
        {
            ...
            IEnumerator DoDefaultMotion()
            {
                ...
                while (time <= duration)
                {
                    ...
                    SetRotation(Mathf.Lerp(InitialRotation, GamePlayManager.MotionUpdater.StartingRotation, p));
                    SetCameraPos(Vector3.Lerp(InitialXY, GamePlayManager.MotionUpdater.StartingPosition, p));
                    SetCameraHeight(Mathf.Lerp(InitialHeight, GamePlayManager.MotionUpdater.StartingHeight, p));

                    time += Time.deltaTime;
                    yield return null;
                }

                SetRotation(GamePlayManager.MotionUpdater.StartingRotation);
                SetCameraPos(GamePlayManager.MotionUpdater.StartingPosition);
                SetCameraHeight(GamePlayManager.MotionUpdater.StartingHeight);
            }
        }
    }
}
