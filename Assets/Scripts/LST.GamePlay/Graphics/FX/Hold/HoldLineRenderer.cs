...
namespace LST.GamePlay.Graphics
{
    ...
    public class HoldLineRenderer : MonoBehaviour
    {
        ...
        public void Setup(ushort scrollGroupID, LongNoteJointCollection jointInfo)
        {
            ...
            var amountInfos = GamePlays.ScrollUpdater.GetProgressions(scrollGroupID, 0.0f, timings);
        ...
        public void DoUpdate()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                var p = GamePlays.ScrollUpdater.GetProgressionSingleFast(_ScrollGroupID, jointNote.ScrollTiming, out var visible);
                if (visible)
                {
                    p = Ease.GameSpaceEase(p, GamePlays.Modifier.NoteEase);
        ...
        {
            return GamePlays.ScrollUpdater.IsScrollRangeVisible(_ScrollGroupID, _MinAmount, _MaxAmount);
        }
    }
}
