        ...
        public void StartDefaultMotion(float duration)
        {
            ...
            IEnumerator DoDefaultMotion()
            {
                ...
                while (time <= duration)
                {
                    ...
                    SetRotation(Mathf.Lerp(InitialRotation, GamePlays.MotionUpdater.StartingRotation, p));
                    SetCameraPos(Vector3.Lerp(InitialXY, GamePlays.MotionUpdater.StartingPosition, p));
                    SetCameraHeight(Mathf.Lerp(InitialHeight, GamePlays.MotionUpdater.StartingHeight, p));

                    time += Time.deltaTime;
                    yield return null;
                }

                SetRotation(GamePlays.MotionUpdater.StartingRotation);
                SetCameraPos(GamePlays.MotionUpdater.StartingPosition);
                SetCameraHeight(GamePlays.MotionUpdater.StartingHeight);
            }
        }
    }
}
