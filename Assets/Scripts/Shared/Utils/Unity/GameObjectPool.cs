...
namespace Utils.Unity
{
    ...
    public class GameObjectInterfacePool<C> : IDisposable where C : IUnityObject
    {
        public int Capacity { get; private set; }
        public int RemainingCount { get; private set; }
        ...
        public GameObjectInterfacePool(GameObjectPoolDescriptor descriptor)
        {
            _Descriptor = descriptor;
            _Descriptor.Capacity = Capacity = Mathf.Max(1, _Descriptor.Capacity);
        ...
        public void Dispose()
        {
            while (_Queue.TryDequeue(out var obj))
            {
                UnityEngine.Object.Destroy(obj.GetUnityObjectReference());
            }
            _Queue.Clear();
        }
    }
